...
add_action( 'wptouch_admin_page_render_wptouch-admin-theme-options', 'foundation_sharing_settings' );
...
function foundation_sharing_init() {
	...
	switch ( $settings->share_location ) {
		case 'top':
			add_filter( 'wptouch_the_content', 'foundation_handle_share_links_top' );
			break;
		case 'bottom':
			add_filter( 'wptouch_the_content', 'foundation_handle_share_links_bottom' );
			break;
	}	
}
...
function foundation_sharing_content() {
	...
	$content = '';
	if ( is_single() && $settings->show_share ) {		
