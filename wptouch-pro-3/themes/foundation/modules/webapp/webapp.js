...
function _wptouchWebAppSetupEachLink( oneLink ) {
	...
	} else if ( targetURL.match( ( /[^\s]+(\.(mp3|mp4|m4a|mov|m4v|wav|aiff))$/i ) ) ) {
...
function wptouchWebAppSetupLinkTrapping() {
	...
	var webAppLinks = jQuery( 'a' ).not( 'ul.no-parent-links li.has_children > a, .no-ajax, .back-button, .fwd-button, .back-to-top, .comment-reply-link, .slide-toggle, .show-hide-toggle, [href^=mailto]' );
