...
function wptouchThemeImgHover() {
	jQuery( '#wptouch-theme-browser img' ).hover( function(){
...
// Functions for dealing with the animation of the progress bars for licenses.
// I can probably clean this stuff up at the end : )
	...
	if ( jQuery( '.uploader' ).length ) {
		jQuery( '.uploader' ).each( function() {
			...
			deleteButton.on( 'click', function( e ) {
				...
				placeHolder.html( '<span class="spinner" style="display:none"></span>' );
				placeHolder.find( '.spinner' ).show();
...
// A little extra gravy to make sure the touchboard news is always full height
function wptouchRefreshOverviewHeight(){
	var windowHeight = ( jQuery( window ).height() - 100 );
	var settingsContainer = jQuery( '#wptouch-settings-content' );
	if ( jQuery( '#touchboard-left' ).height() > windowHeight ) {
		jQuery( settingsContainer ).height( 'auto' );
	} else {
		jQuery( settingsContainer ).height( windowHeight );
	}
}
...
function wptouchSetInputColor( inputObject, color ) {
...
function wptouchLoadTouchBoardImages() {
	var touchboardCarousel = jQuery( '#touchboard-carousel' );
	if ( touchboardCarousel.length ) {
		var ajaxParams = {};
		
		wptouchAdminAjax( 'touchboard-images', ajaxParams, function( result ) {
			var images = jQuery.parseJSON( result );
			if ( images ) {
				for ( var i = 0; i < images.length; i++ ) {
					jQuery( '.carousel-inner' ).prepend( '<div class="item"><img src="' + images[i] + '"></div>' );
				}	
			}


			var carouselItem = jQuery( '.carousel-inner' ).find( '.item' );			
			carouselItem.removeClass( 'active' );
			carouselItem.first().addClass( 'active' );
			carouselItem.last().remove();
			
			touchboardCarousel.addClass( 'carousel' ).carousel({
				interval: 5000,
				pause: 'hover'
			});

			wptouchHandleCarousels();
		});	
	}
}

function wptouchHandleTouchBoardBoxes(){
	jQuery( '#help-boxes' ).find( 'li:not(li.updates)' ).each( function(){ 
...
function wptouchAdminUpdateMenuSetupPages() {
	...
	jQuery( '#menu-icons-menus > .wptouch-section' ).load( document.location.href + ' #menu-icons-menus #pack-set-menu-area, #menu-icons-menus #default-trash-area', function() {
...
function wptouchSetupNews() {
	...
	if ( newsDiv.length ) {
		...
		wptouchAdminAjax( 'load-news', ajaxParams, function( result ) {
			...
			jQuery( '#ajax-news .nano' ).nanoScroller({
				preventPageScrolling: true
			});	
		});
	}
